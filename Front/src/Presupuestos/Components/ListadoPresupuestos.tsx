...
export default function ListadoPresupuestos(props: propsListadoPresupuestos) {
  ...
  function crearModelo(valores: presupuestoModel) {
    ...
    json[0] = {
      ...
      IdProducto: valores.productos[0].id_producto.toString(),
    ...
    for (let i = 1; i < valores.productos.length; i++) {
      json[i] = {
        ...
        Descuento: "",
        Fecha: "",
        IdProducto: valores.productos[i].id_producto.toString(),
  ...
  const botones = (id: number) => (
    <>
      <Button style={{ marginRight: "1rem", marginLeft: "1rem" }} className="btn btn-info" onClick={() => handleExport(id)}>
        Exportar
      </Button>
      <Button
  ...
  return (
    <>
      <Button
        style={{ marginBottom: "1rem", marginLeft: "65.75rem", marginTop: "-85px" }}
        onClick={() => {
          showModal();
        }}
        className="btn btn-transparent"
      ...
      <Modal title="Generar Presupuesto" width={1150} open={open} footer={null} centered onCancel={showModal}>
        <p>
          <Presupuesto setFlagModal={showModal} setFlagListado={props.setFlag} />
        </p>
      </Modal>
      <Modal title="Editar Presupuesto" width={1150} open={edit} footer={null} centered onCancel={showEdit}>
        <p>
          <EditarPresupuesto id={id!} setFlagModal={showEdit} setFlagListado={props.setFlag} />
        </p>
      </Modal>
